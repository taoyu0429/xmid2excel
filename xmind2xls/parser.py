...
def parse_model2suite(suite_dict):
    model2suite = TestSuite()
    model2suite.name = suite_dict['title']
    model2suite.details = suite_dict['note']
    model2suite.sub_suites = []

    for suite_model2 in suite_dict['topics']:
        model2suite.sub_suites.append(parse_fuctionsuite(suite_model2))

    '''
    testsuite.testcase_list = [] 
    logging.debug('start to parse a testsuite: %s', testsuite.name)
    
    for cases_dict in suite_dict['topics'][0].get('topics', []):
        for case in recurse_parse_testcase(cases_dict):
            testsuite.testcase_list.append(case)
...
def parse_fuctionsuite(suite_dict):

    functionsuite = TestSuite()
    functionsuite.name = suite_dict['title']
    functionsuite.details = suite_dict['note']
    functionsuite.sub_suites = []
    for suite_dict in suite_dict['topics']:
        functionsuite.sub_suites.append(parse_checkpointsuite(suite_dict))
    return functionsuite


def parse_checkpointsuite(suite_dict):
    checkpointsuite = TestSuite()
    checkpointsuite.name = suite_dict['title']
    checkpointsuite.details = suite_dict['note']

    checkpointsuite.testcase_list = []
    logging.debug('start to parse a testsuite: %s', checkpointsuite.name)

    for cases_dict in suite_dict.get('topics', []):
        for case in recurse_parse_testcase(cases_dict):
            checkpointsuite.testcase_list.append(case)

    logging.debug('testsuite(%s) parsing complete: %s', checkpointsuite.name, checkpointsuite.to_dict())
    return checkpointsuite
...
def parse_a_testcase(case_dict, parent):
    ...
    step_dict_list.insert(0, testcase.name)
    if step_dict_list:
